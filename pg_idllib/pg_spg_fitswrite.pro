...
;
;       cntfilename: filename with extension (.fit)               
...
;       11-FEB-2004 problem: now looks at aspect solution data for
;                   semical data-> not good if you want night spgs too
;                   Therefore changed the routine as to not ask for
;                   semical data if not needed
;
; AUTHOR
;       Paolo Grigis, Institute for Astronomy, ETH, Zurich
;-

PRO pg_spg_fitswrite,time_intv,time_bin=time_bin,energy_band=energy_band $
    ,cntfilename=cntfilename,front=front,rear=rear, segment=segment $
...
;--------------------------------------------------
   ...
   IF keyword_set(preview) THEN BEGIN
       ...
       IF n_elements(cntfilename) EQ 1 THEN BEGIN
           print,'Save to file  :'
           print,'Count spg     : '+cntfilename
...
;---------------------------------------------------------
     ...
     IF keyword_set(scfilename) THEN BEGIN
