...
;      Here, the "s" (slowing down) frequency is returned
;
...
;
; CALLING SEQUENCE:
;      y=pg_collnu_s(v,coulomblog=coulomblog,dens=dens,temp=temp
...
FUNCTION pg_collnu_s,v,logfactor=logfactor,dens=dens,temp=temp $
                    ,massa=massa,massb=massb,charga=charga,chargb=chargb
...
;nus=nuab/temp*(massa+massb)*pg_chandrag(xb)/v

return,nuab/temp*(massa+massb)*pg_chandrag(xb)/v
