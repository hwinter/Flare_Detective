...
;using spectro_plot
;
PRO spg_examiner_plotspg,spgstr=spgstr,drawwidget
...
IF spgstr.needupdate EQ 1 THEN BEGIN 
    ...
    IF spgstr.background EQ 1 THEN BEGIN
        ...
        ascspg=hsi_background_subtract(spg=spgstr.scspg $
                                             ,timerange=spgstr.backtime) 
    ENDIF $
    ELSE BEGIN
    ...
    IF spgstr.integrated EQ 1 THEN BEGIN        
        ...
        ascspg=spg_integrate(ascspg,xint=spgstr.xint,yint=spgstr.yint)
        
    ENDIF

     aspgstr={aspg:aspg,ascspg:ascspg}
     spgstr=join_struct(aspgstr,spgstr)

ENDIF

IF spgstr.spgtype EQ 0 THEN $
    spg=spgstr.aspg $
ELSE  BEGIN
    spg=spgstr.ascspg
ENDELSE
...
PRO spg_examiner_plotspectrum,spgstr,tindex,fitfun=fitfun,spectrum=spectrum $
                             ,title=title,oplotfit=oplotfit

    IF spgstr.spgtype EQ 0 THEN BEGIN 
        spg=spgstr.aspg
        ytitle='Counts'
    ENDIF ELSE BEGIN
        spg=spgstr.ascspg
...
;    IF (spgstr.fit EQ 1) AND (spgstr.spgtype EQ 1) THEN BEGIN
...
;
PRO spg_examiner_plotbox,newcoor,oldcoor,sides=sides,noerase=noerase $
                        ,nodraw=nodraw
...
;
FUNCTION spg_examiner_outtext,spgstr

startime=anytim(spgstr.aspg.x[0],/vms)
endtime =anytim(spgstr.aspg.x[n_elements(spgstr.aspg.x)-1],/vms)

IF spgstr.spgtype EQ 0 THEN datatype='Counts' ELSE datatype='Semicalibrated'
...
;fitting
FUNCTION spg_examiner_fit,spgstr,time,een=een,yy=yy,eerrors=eerrors
...
CASE ev.ID OF
    ...
    widget_info(ev.top,find_by_uname='commands2') : BEGIN
        ...
        CASE ev.value OF
            ...
            0 : BEGIN           ;spg type toggle
            
                IF spgstr.inputspgtype EQ 0 THEN BEGIN
                    spgstr.spgtype=1-spgstr.spgtype
                ENDIF
                
                ;drawwidget=widget_info(ev.top,find_by_uname='drawwin')
                spg_examiner_plotspg,spgstr=spgstr,drawwidget

                textwindow=widget_info(ev.top,find_by_uname='frameinfo')
                widget_control,textwindow $
                              ,set_value=spg_examiner_outtext(spgstr)
            ...
            5: BEGIN ;fit max spectrum
                ...
                fitpar=spg_examiner_fit(spgstr,een=spg.y,eerrors=espectrum $
                                       ,yy=spectrum)
                ...
                spg_examiner_plotspectrum,spgstr,fitfun=fitpar.fun $
            ...
            6 : BEGIN ; set fit start par guesses
                ...
                xmanager,'spg_examiner',basesetfitpar,/no_block
...
;
        ...
        IF ev.press EQ 1 THEN BEGIN
            ...
            IF status.last THEN BEGIN
                ...
                spg_examiner_plotbox,status.boxcoor,status.oldcoor,/nodraw $
                                     ,sides=newsides
            ENDIF
        ...
        IF status.dragging EQ 1 THEN BEGIN
            ...
            IF status.first THEN BEGIN
                ...
                spg_examiner_plotbox,status.boxcoor,status.oldcoor,/noerase $
                                   ,sides=newsides
                status.first=0
            ...
            ELSE BEGIN
                ...
                spg_examiner_plotbox,status.boxcoor,status.oldcoor $
                                   ,sides=newsides
            ENDELSE
        ...
        IF ev.release EQ 4 THEN BEGIN
            ...
            textwindow=widget_info(ev.top,find_by_uname='frameinfo')
            widget_control,textwindow,set_value=spg_examiner_outtext(spgstr)
            
            spg_examiner_plotspg,spgstr=spgstr,ev.id                          
    ...
    IF status.operation EQ 'yslice' THEN BEGIN
        ...
        IF ev.press EQ 1 THEN BEGIN
            ...
            IF (spgstr.fit EQ 1) AND (spgstr.spgtype EQ 1) THEN BEGIN
                fitpar=spg_examiner_fit(spgstr,x1)
                spgstr.fitpar=fitpar.par
                fitfun=fitpar.fun
                textwindow=widget_info(ev.top,find_by_uname='frameinfo')
                widget_control,textwindow $
            ...
            spg_examiner_plotspectrum,spgstr,tindex[0],fitfun=fitfun $
                                     ,oplotfit=oplotfit
    ...
    IF status.operation EQ 'bgselect' THEN BEGIN
        ...
        IF ev.release EQ 1 THEN BEGIN
                ...
                IF status.clicks EQ 2 THEN BEGIN
                    ...
                    spg_examiner_plotspg,spgstr=spgstr,spglc
...
;Main procedure
;
PRO spg_examiner,spg=spg,scspg=scspg

;variable initialisation stuff
;

ist=0; means both spectrograms
st=0; counts

IF NOT exist(scspg) THEN BEGIN
...
;
    ...
    values=['Spectrogram units','Background select','Clear Background' $
...
;text widget
;
    text=widget_text(menu1,value=spg_examiner_outtext(spgstr) $
...
spg_examiner_plotspg,spgstr=spgstr,draw

xmanager,'spg_examiner',root,/no_block
