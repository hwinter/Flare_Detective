...
;
; PURPOSE: 
;      returns the value of the Coulomb Diffusion term
...
;       21-NOV-2005 added density keyword (formerly assumed to be 10d10)
...
FUNCTION pg_clmbdiff_dimless,y,yt,dtime,density=density,clmblog=clmblog $
   ...
   res=scale_factor*4d *nu_0*10d^(0.5*y)*mc2^(-1.5)*2^(-1.5)*elmass^(1.5) $
      *(pg_hubapsi(x))/x*dtime
