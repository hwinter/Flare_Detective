...
;      "theory.tex"
;      In addition, we have an external electric field apllied,
;      *** assuming it works only on the test particles ***  (???make sense???)
;
;      ++++++++++ NOTATION: y=log_10(e)
...
;             indefinitely)
;      efield: external electric field strength 
;   
; OUTPUTS:
;      
;      
...
PRO pg_cn_efieldacc,startdist=startdist,ygrid=ygrid,ytherm=ytherm,niter=niter $
                            ,dt=dt,lastdist=lastdist,diagnostic=diagnostic,efield=efield

N=n_elements(startdist)
dt=fcheck(dt,1.)
dx=ygrid[1]-ygrid[0]

IF NOT exist(efield) THEN BEGIN
   print,'Please input the elctric field strength'
   return
ENDIF
...
cefa=pg_efacc_dimless(ygrid,ytherm,efield=efield)
cefaplus=shift(cefa,-1) 
cefaminus=shift(cefa,1) 

cefaminus[0]=pg_efacc_dimless(ygrid[0]-dx,ytherm,efield=efield)
cefaplus[N-1]=pg_efacc_dimless(ygrid[N-1]+dx,ytherm,efield=efield)
