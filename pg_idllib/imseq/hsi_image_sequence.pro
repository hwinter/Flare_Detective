...
; CALLING SEQUENCE:
;
;        hsi_image_sequence,time_intv,imtime=imtime,onefile=onefile
...
;                default: 60 seconds
;        onefile: keyword to force output to a single file
;                 (which can then be read using hsi_pg_fits2map)
;
; OUTPUT: to a fit file (or files)        
;
; VERSION:
...
;       30-JAN-2002 changed output again, allowing the user to produce
;       one fit file only instead of many
;
;
; AUTHOR
...
PRO  hsi_image_sequence,time_intv,imtime=imtime,energy_band=energy_band $
     ...
     ,pmtras_diagnostic=pmtras_diagnostic,onefile=onefile
...
ENDIF ELSE BEGIN
   ...
   FOR i=0,N-1 DO $
       ...
       IF i EQ 0 THEN $
         im->fitswrite,fitsfile=directory+filename,/create $
       ELSE $
         im->fitswrite,fitsfile=directory+filename
