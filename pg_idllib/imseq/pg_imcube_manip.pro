...
; VERSION:
;
;        29-JAN-2007 written, based on imseq_manip
;
; AUTHOR
...
;.comp pg_imcube_manip
PRO pg_imcube_manip_plotmap_test

  pg_imcube_manip,res,spg=spg
...
;
PRO pg_imcube_manip_plotmap,mapstr,drawwidget
...
IF mapstr.totalplot EQ 1 THEN BEGIN
   ...
   FOR j=0,mapstr.nenergy-1 DO BEGIN
      ...
      FOR i=0,mapstr.ntime-1 DO BEGIN
         ...
         pg_imcube_manip_plotmap,mapstr2,drawwidget
...
;
PRO pg_imcube_manip_plottimev,mapstr,drawwidget
...
;graphic window
;
PRO pg_imcube_manip_plotbox,newcoor,oldcoor,sides=sides,noerase=noerase $
                       ,nodraw=nodraw
...
;
FUNCTION pg_imcube_manip_outtext,mapstr
...
;        pg_imcube_manip_plotmap,mapstr,drawwidget

;        IF mapstr.plottimev THEN BEGIN
;           drawwidget=widget_info(ev.top,find_by_uname='timevwin')
;           pg_imcube_manip_plottimev,mapstr,drawwidget
;        ENDIF
...
;     END
    ...
    widget_info(ev.top,find_by_uname='selectfitm') : BEGIN
       ...
       widget_control,textwindow,set_value=pg_imcube_manip_outtext(mapstr)
    ...
    widget_info(ev.top,find_by_uname='commands') : BEGIN
        ...
        CASE ev.value OF
            ...
            0 : BEGIN ;Draw total Image
                ...
                IF mapstr.plottimev THEN pg_imcube_manip_plottimev,mapstr,drawtimevwidget
           ...
           END
            ...
            6 : BEGIN ; 'total plot'
               ...
               IF mapstr.totalplot THEN $
                  widget_control,drawwidget,draw_xsize=512.*mapstr.ntime/2,draw_ysize=512.*mapstr.nenergy/2 $
               ELSE $
                  widget_control,drawwidget,draw_xsize=512,draw_ysize=512
...
;
        ...
        IF ev.press EQ 1 THEN BEGIN
            ...
            IF status.last THEN BEGIN
                ...
                pg_imcube_manip_plotbox,status.boxcoor,status.oldcoor,/nodraw $
                                     ,sides=newsides
            ENDIF
        ...
        IF status.dragging EQ 1 THEN BEGIN
            ...
            IF status.first THEN BEGIN
                ...
                pg_imcube_manip_plotbox,status.boxcoor,status.oldcoor,/noerase $
                                   ,sides=newsides
                status.first=0
            ...
            ELSE BEGIN
                ...
                pg_imcube_manip_plotbox,status.boxcoor,status.oldcoor $
                                   ,sides=newsides
            ENDELSE
        ...
        IF ev.release EQ 4 THEN BEGIN
            ...
            pg_imcube_manip_plotmap,mapstr,ev.id
...
;text widget
;
    text=widget_text(menu1,value=pg_imcube_manip_outtext(mapstr),ysize=12 $
                    ,uname='frameinfo')
;end text widget
...
pg_imcube_manip_plotmap,mapstr,draw
IF plottimev EQ 1 THEN pg_imcube_manip_plottimev,mapstr,draw2

xmanager,'pg_imcube_manip',root,/no_block
