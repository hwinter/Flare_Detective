...
;.comp pg_imcube_spg_manip
PRO pg_imcube_spg_manip_plotmap_test

  pg_imcube_spg_manip,res,spg=spg
...
;
PRO pg_imcube_spg_manip_plotmap,mapstr,drawwidget
...
IF mapstr.totalplot EQ 1 THEN BEGIN
   ...
   FOR j=0,mapstr.nenergy-1 DO BEGIN
      ...
      FOR i=0,mapstr.ntime-1 DO BEGIN
         ...
         pg_imcube_spg_manip_plotmap,mapstr2,drawwidget
...
;
PRO pg_imcube_spg_manip_plottimev,mapstr,drawwidget
...
;graphic window
;
PRO pg_imcube_spg_manip_plotbox,newcoor,oldcoor,sides=sides,noerase=noerase $
                       ,nodraw=nodraw
...
;
FUNCTION pg_imcube_spg_manip_outtext,mapstr
...
;
PRO pg_imcube_spg_manip_event,ev
...
;        pg_imcube_spg_manip_plotmap,mapstr,drawwidget

;        IF mapstr.plottimev THEN BEGIN
;           drawwidget=widget_info(ev.top,find_by_uname='timevwin')
;           pg_imcube_spg_manip_plottimev,mapstr,drawwidget
;        ENDIF
...
;     END
    ...
    widget_info(ev.top,find_by_uname='selectfitm') : BEGIN
       ...
       widget_control,textwindow,set_value=pg_imcube_spg_manip_outtext(mapstr)
 ...
 ;       ENDIF
    ...
    widget_info(ev.top,find_by_uname='commands') : BEGIN
		...
		widget_control,drawwidget,draw_xsize=mapstr.widgxsize $
                ...
                IF mapstr.plottimev THEN pg_imcube_spg_manip_plottimev,mapstr,drawtimevwidget
...
;                baselc=widget_base(title='Total plot',/row,group_leader=ev.top)
;                drawlc=widget_draw(baselc,xsize=700,ysize=500 $
;                                  ,uname='drawlc')

;                widget_control,baselc,/hourglass

;                imcube_ltc,mapstr.imseq,lev=0,tim=tim,lc=lc,box=mapstr.selcoor $
;                         ,err=err
...
;
        ...
        IF ev.press EQ 1 THEN BEGIN
            ...
            IF status.last THEN BEGIN
                ...
                pg_imcube_spg_manip_plotbox,status.boxcoor,status.oldcoor,/nodraw $
                                     ,sides=newsides
            ENDIF
        ...
        IF status.dragging EQ 1 THEN BEGIN
            ...
            IF status.first THEN BEGIN
                ...
                pg_imcube_spg_manip_plotbox,status.boxcoor,status.oldcoor,/noerase $
                                   ,sides=newsides
                status.first=0
            ...
            ELSE BEGIN
                ...
                pg_imcube_spg_manip_plotbox,status.boxcoor,status.oldcoor $
                                   ,sides=newsides
            ENDELSE
        ...
        IF ev.release EQ 4 THEN BEGIN
            ...
            pg_imcube_spg_manip_plotmap,mapstr,ev.id
...
;Main procedure
;
PRO pg_imcube_spg_manip,ptrimcube,spg=spg
...
mapstr={imseq:ptr,totmap:ptr_new(),minfr:0,maxfr:n_elements(ptr)-1 $
       ...
       ,show_spg:0,spg:spg,widgxsize:512,widgysize:512}
...
;text widget
;
    text=widget_text(menu1,value=pg_imcube_spg_manip_outtext(mapstr),ysize=12 $
                    ,uname='frameinfo')
;end text widget
...
;
    draw=widget_draw(drawsurf1,xsize=mapstr.widgxsize,ysize=mapstr.widgysize $
		    ,uname='drawwin',/scroll,x_scroll_size=768,y_scroll_size=900)
...
pg_imcube_spg_manip_plotmap,mapstr,draw
IF plottimev EQ 1 THEN pg_imcube_spg_manip_plottimev,mapstr,draw2

xmanager,'pg_imcube_spg_manip',root,/no_block
