...
;  25-AUG-2008 corrected bug for the case of non-quadratic arrays PG
;
;-
...
IF sp[0] NE 2 THEN BEGIN 
   dopseudocomp=1 
   IF n_elements(nsquare) EQ 0 THEN nsquare=1L ELSE nsquare=nsquare>1L
ENDIF $
ELSE BEGIN 
   dopseudocomp=0
...
yind=(ind/sm[1])>(nsquare)<(sm[2]-nsquare-1)
