...
;.comp pg_xrt_manip.pro


;
;plot the map on the screen, with the selected attributes
;
PRO pg_xrt_manip_plotmap,mapstr,drawwidget
...
;
PRO pg_xrt_manip_plottimev,mapstr,drawwidget
...
;graphic window
;
PRO pg_xrt_manip_plotbox,newcoor,oldcoor,sides=sides,noerase=noerase $
                       ,nodraw=nodraw
...
;
FUNCTION pg_xrt_manip_outtext,mapstr
...
imsize=size((*mapstr.imseq)),/dimension)

outtext=['XRT image', $
         'Image type: '+mapstr.index[i].ec_imty_, $
         'FW 1: '+mapstr.index[i].ec_fw1_, $
         'FW 2: '+mapstr.index[i].ec_fw2_, $
         'Obs. time: '+anytim(mapstr.index[i].date_obs,/vms), $
         'Exposure time: '+string(mapstr.index[i].exptime), $
         ' ',$
...
CASE ev.ID OF 
    ...
    widget_info(ev.top,find_by_uname='selectim') : BEGIN
       ...
       pg_xrt_manip_plotmap,mapstr,drawwidget

       textwidget=widget_info(ev.top,find_by_uname='frameinfo') 
       widget_control,textwidget,set_value=pg_xrt_manip_outtext(mapstr)
...
;                IF mapstr.plottimev THEN pg_xrt_manip_plottimev,mapstr,drawtimevwidget
...
;
        ...
        IF ev.press EQ 1 THEN BEGIN
            ...
            IF status.last THEN BEGIN
                ...
                pg_xrt_manip_plotbox,status.boxcoor,status.oldcoor,/nodraw $
                                     ,sides=newsides
            ENDIF
        ...
        IF status.dragging EQ 1 THEN BEGIN
            ...
            IF status.first THEN BEGIN
                ...
                pg_xrt_manip_plotbox,status.boxcoor,status.oldcoor,/noerase $
                                   ,sides=newsides
                status.first=0
            ...
            ELSE BEGIN
                ...
                pg_xrt_manip_plotbox,status.boxcoor,status.oldcoor $
                                   ,sides=newsides
            ENDELSE
        ...
        IF ev.release EQ 4 THEN BEGIN
            ...
            widget_control,textwindow,set_value=pg_xrt_manip_outtext(mapstr)
            
            pg_xrt_manip_plotmap,mapstr,ev.id                          
...
;Main procedure
;
PRO pg_xrt_manip,data,index,quiet=quiet,outptrimseq=ptr,outcat=cat

;check whether xrtimseq consists of filenames or pointers

n_im=(size(data))[3]
n_cat=n_elements(index)

IF (n_cat NE n_im) OR (size(index,/tname) NE 'STRUCT') THEN BEGIN 
   print,'Invalid input'
   RETURN
ENDIF


tarr=anytim(index.date_obs)
imind=sort(tarr)

mapstr={imseq:ptr_new(data) $
        ,imind:imind $
        ,nmap:n_im $
        ,selim:0 $
        ,limb:1 $
...
;
    ...
    images=strtrim(string(indgen(n_im)),2)
...
;text widget
;
    text=widget_text(menu1,value=pg_xrt_manip_outtext(mapstr),ysize=12 $
                    ,uname='frameinfo')
;end text widget
...
pg_xrt_manip_plotmap,mapstr,draw
;pg_xrt_manip_plottimev,mapstr,draw2

xmanager,'pg_xrt_manip',root,/no_block
