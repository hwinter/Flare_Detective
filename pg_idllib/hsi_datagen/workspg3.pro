...
;        24-APR-2002 start to convert to use the new plot_spg
;        routine... 
;     
;
; AUTHOR
...
WHILE (!mouse.button NE 4) DO BEGIN
    ...
    IF !mouse.button EQ 1 THEN BEGIN
        ...
        xarrint=where(abs(plotx-(plotx[0]+xint)) EQ $
                      min(abs(plotx-(plotx[0]+xint))))
        ...
        'v'  : BEGIN
            ...
            WHILE (!mouse.button NE 4) DO BEGIN
                ...
                IF !mouse.button NE 0 THEN BEGIN
                    wset,pwin
                    plot,ploty,plotim[xarrint,*],yrange=[0.1,max(plotim[xarrint,*])] $
