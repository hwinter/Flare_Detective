...
; 11-DEC-2006 recycled for further use, but having a problem with the roi
;             --> fix bug and see what happens
;-
...
IF size(map,/type) EQ 10 THEN BEGIN
   ...
   FOR i=0L,np-1 DO BEGIN
      ...
      x[i]=xx
      y[i]=yy
      xerr[i]=fcheck(xxerr,-1)
      yerr[i]=fcheck(yyerr,-1)
      peakvalue[i]=pp
      fitout[i]=ptr_new(ffitout)
   ENDFOR
