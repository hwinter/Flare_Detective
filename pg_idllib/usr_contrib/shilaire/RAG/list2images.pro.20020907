...
IF time_interval(0) GE anytim('2001/01/01 00:00:00') THEN rag_root='/global/helene/local/rag/observations/' $
ELSE rag_root='/global/hercules/data1/rag/observations/'
;initialize output
ragfile=-1
...
FOR i=0,nbursts-1 do begin
	...
	IF ((TODO NE 2) AND ( (anytim(time_interval(1))-anytim(time_interval(0))) LE MAXTIMEINTV) ) THEN BEGIN
		...
		IF DATATYPE(ragfiles) EQ 'INT' THEN BEGIN
