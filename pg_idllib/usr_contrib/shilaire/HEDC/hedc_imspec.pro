...
; EXAMPLE:
;
;	hedc_imspec, -1, [2,8,12], fitsfile=fil
;	hedc_imspec, -1, [2,8,12],fitsfile=fil,SKIP=2
;	set_plot,'Z' & hedc_imspec, -1, [2,8,12],fitsfile=fil, pngfile='test.png' & X
;
...
;	V9: (PSH, 2004/09/21): some more successful debugging... And replaced source numbering by lettering... I'm still amazed at my cleverness... ;-)
;
;----------------------------------------------------------------------------------------------------------------------------------------
...
resolve_routine,'mpfit',/EITHER,/COMPILE_FULL_FILE,/NO_RECOMPILE
	...
	tintvs=anytim(infoctrl.im_time_interval);tintvs=infoctrl.times_arr
...
!P.MULTI=[n_yplots*n_tintvs-1,n_tintvs,n_yplots,0,1] & plot_map,{data: DIST(100), xc:0, yc:0, dx:1, dy:1, time:anytim(/TAI,'2000/02/26 01:00:00'), id:'DUMMY'}
	...
	ytit='Cts/s/det'
		;ytit='Corrected !CCts/s/det'
	utplot,ostimes,TOTAL(osrates[0:1,*],1),color=1,ytit=ytit,/YLOG,yr=[1,10000],xtit='',charsize=1.5,xstyle=1,xmar=[6,1],ymar=[2,1]
	...
	FOR i=0L,n_tintvs-1 DO BEGIN
		...
		IF ichooser LT 0 THEN BEGIN
			src='blabla'
			src_eband=INTARR(njchooser)	;number of sources in each eband chosen to test for sources...
			FOR j=0,njchooser-1 DO BEGIN
				...
				IF datatype(newsrc) EQ 'STC' THEN BEGIN	;are there any new sources ?
					; ...if yes: check for each one whether it is not too similar to previous ones. If not, add it to src! But beware, no more than n_src_max sources per eband!
					new_src_taken=0
					FOR k=0,N_ELEMENTS(newsrc)-1 DO BEGIN
						;if no src already exist, src is now newsrc[0]...
						IF datatype(src) NE 'STC' THEN BEGIN
							src=newsrc[0]
							new_src_taken=new_src_taken+1
						ENDIF ELSE BEGIN
							;is dissimilar enough from other src ?
							ss=WHERE( ((src.xmax-newsrc[k].xmax)^2 LE (src.gra*src.grb/4.)) AND ((src.ymax-newsrc[k].ymax)^2 LE (src.gra*src.grb/4.)) AND (src.gra*src.grb/newsrc[k].gra/newsrc[k].grb GE 1/2.)  AND (src.gra*src.grb/newsrc[k].gra/newsrc[k].grb LE 2.) ) 
							toosimilar=(ss[0] NE -1)
							IF NOT toosimilar THEN BEGIN
								src=[src,newsrc[k]]
								new_src_taken=new_src_taken+1							
							ENDIF
						ENDELSE
						IF new_src_taken GE n_src_max THEN BREAK	
					ENDFOR
					src_eband[j]=new_src_taken					
				ENDIF ELSE src_eband[j]=0
			ENDFOR;j
			IF datatype(src) EQ 'STC' THEN n_src=N_ELEMENTS(src) ELSE n_src=0
		ENDIF
		...
		FOR j=0,ndisp-1 DO BEGIN
			...
			FOR k=0,src_eband[j]-1 DO BEGIN
				...
				XYOUTS,/DATA,infoctrl.xyoffset[0]+infoctrl.pixel_size[0]*(src[cur_src].xmax-infoctrl.image_dim[0]/2+6.5),infoctrl.xyoffset[1]+infoctrl.pixel_size[1]*(src[cur_src].ymax-infoctrl.image_dim[1]/2-4.5),STRING(BYTE(cur_src+65)),color=4
...
;now, add a small margin to the left and on top, with some more info...
	...
	ENDFOR
	XYOUTS,10,(n_yplots-0.8)*basesize,/DEVICE,"cts/s/det",ORIENTATION=90
