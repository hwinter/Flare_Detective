...
;
; EXAMPLE: 
;	list=list_all_rhessi_flares_above_threshold(['2002/02/13','2003/02/18'],/STATES,/DISTANCE,thresh=3e-6)
...
FUNCTION list_all_rhessi_flares_above_threshold, time_intv, threshold=threshold, STATES=STATES, DISTANCE=DISTANCE
	
	IF NOT KEYWORD_SET(threshold) THEN threshold=1e-5	;M-class
				...
				IF ((fl[j].PEAK_TIME GE cur_intv[0]) AND (fl[j].PEAK_TIME LE cur_intv[1])) THEN BEGIN
					...
					IF datatype(goesdata) NE 'UND' THEN BEGIN
						...
						IF themax GE threshold THEN BEGIN
							...
							IF KEYWORD_SET(STATES) THEN BEGIN
								oso=hsi_obs_summary()
							...
							ENDIF
							IF KEYWORD_SET(DISTANCE) THEN BEGIN	;FOR NOW, MUST BE RUN WITH NFS ACCESS TO /global/hercules/users !
								;;;;need to use HEDC, if available...
								;;;;find whether the flare is an HEDC event.
								;;;;if it is, take the flare position...
								;;;tmp=anytim(fl[j].PEAK_TIME,/EX)
								;;;eventcode='HXS'+int2str(tmp[6],1)+int2str(tmp[5],2)+int2str(tmp[4],2)+int2str(tmp[0],2)+int2str(tmp[1],2)
								;;;SPAWN,"java -cp /global/hercules/users/shilaire/HEDC/util:/global/hercules/users/hedc/dm/classes/dm.jar:/global/hercules/users/hedc/dm/classes/sc_lib.jar query S12_HLE_CODE eq "+eventcode+" F04_HLE_DISTANCESUN",res,err_res, exit_status=exit_status
