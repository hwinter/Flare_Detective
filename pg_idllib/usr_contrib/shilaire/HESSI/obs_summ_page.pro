...
;
;	/ENERGY: the FRONT (only!) decimation channels will be converted into energies, using Paolo's routine.
;
...
;	Modified: 2006/01/12: Replaced call to pg_plotgoes.pro to plot GOES data.
;	Modified: 2006/04/18: Minor modification (if keyword ENERGY is set, writes down ENERGY instead of CHANNEL in ytitle for decimation profile).
;	Modified: 2006/04/24: Monitor rate livetimes are now heuristically corrected for datagaps, and should be closer to livetime_counter (the value important for pile-up corrections) -- as per http://hesperia.gsfc.nasa.gov/~richard/livetime_discuss.pro
;
;-
...
IF KEYWORD_SET(SEGMENTS) THEN BEGIN
	...
	utplot,qlotimes-qlotimes[0],qlorates[0,*],qlotimes[0], trange=anytim(time_intv,/yoh), xtitle='', xstyle=1, $
		ytitle='MON. counts/s !C(FRONT SEGMENTS)',color=2,yrange=yrng, YLOG=1-LIN, $
...
ENDIF ELSE BEGIN
	rd_goes,gtimes,gdata,trange=anytim(time_intv,/yoh)	;, gsat=10
	IF datatype(gdata) NE 'UND' THEN BEGIN
		ssg=WHERE( (gtimes GE anytim(time_intv[0])) AND (gtimes LE anytim(time_intv[1])))
		IF ssg[0] NE -1 THEN BEGIN
			clear_utplot
			utplot, gtimes[ssg], gdata[ssg,0], thick=2, color=7, xmargin=[10,2],ymargin=[1,1], /YLOG, yrange=[1d-8,MAX(gdata[ssg,0])], ytit='GOES!C[W m!U-2!N]',charsize=charsize,xstyle=1,timerange=time_intv,xtit='',YTICKNAME=['A','B','C','M','X','X10','X100']
			outplot,gtimes[ssg], gdata[ssg,1], color=12
		ENDIF ELSE PLOT,FINDGEN(10),/NODATA, XTICKNAME=REPLICATE(' ',10),YTICKNAME=REPLICATE(' ',10), ytit='GOES!C[W m!U-2!N]', xmargin=[10,2],ymargin=[1,1], charsize=charsize
	ENDIF ELSE PLOT,FINDGEN(10),/NODATA, XTICKNAME=REPLICATE(' ',10),YTICKNAME=REPLICATE(' ',10), ytit='GOES!C[W m!U-2!N]', xmargin=[10,2],ymargin=[1,1], charsize=charsize
	...
	IF GOES GE 2 THEN BEGIN
		...
		IF datatype(gev) EQ 'STC' THEN BEGIN
			FOR i=0L,N_ELEMENTS(gev)-1 DO BEGIN
				PLOTS,[anytim(gev[i].START_TIME),anytim(gev[i].END_TIME)],[1d-8,1d-8],/DATA,color=5,thick=3
				label_plot,/XDATA,/YDATA,anytim(gev[i].PEAK_TIME),2d-7,gev[i].CLASS+'/'+strn(gev[i].NOAA_AR),color=5
				label_plot,/XDATA,/YDATA,anytim(gev[i].PEAK_TIME),2d-8,strn(gev[i].LOCATION[0],format='(f10.0)')+'/'+strn(gev[i].LOCATION[1],format='(f10.0)'),color=5				
			ENDFOR
...
ENDIF ELSE BEGIN
	...
	IF KEYWORD_SET(ENERGY) THEN ytit='DECIMATION!CENERGY or weight' ELSE ytit='DECIMATION!CCHANNEL or weight'
	yrange=[1,1000]
	utplot,flagtimes-flagtimes[0],flagdata[wantedflags[0],*],flagtimes[0],charsiz=charsize,xmargin=[10,2],ymargin=[1,1],xtitle='', xstyle=1, color=250,$
		yrange=yrange,ytitle=ytit,ystyle=1,/YLOG,/NODATA
	FOR i=0,N_ELEMENTS(wantedflags)-1 DO BEGIN
		fdata=flagdata(wantedflags[i],*)
