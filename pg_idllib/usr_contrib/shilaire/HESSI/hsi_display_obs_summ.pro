...
;		counts=3 : 3-12, 12-25, 25-100, 100-17000 keV energy bands
;		counts=4 : as above, minus the 12-25 keV band.
;
; /mod_var
;
; /part_rates
...
;
; filename : will output result in this path under PNG format unless /jpg keyword
;			is also set, in which case output will be in JPEG format 
;	!!!: here, it is pointless to have more than one Obs. Summ. object displayed, 
;		as they'll all be saved under the same filename...
;
;OUTPUT:
...
;KEYWORDS:
;	see above
;	/Zbuffer	: to use Z-buffering...
;	plus most keywords normally allowed in calls to plot
;
;EXAMPLE:
...
; Modified 2001/12/18 : added _extra keyword, and 12-25 keV energy bands in counts
;
;-
...
IF NOT KEYWORD_SET(Zbuffer) THEN window,0,xs=630,ys=512 ELSE BEGIN
	old_device=!D.NAME
	set_plot,'Z',/COPY
	DEVICE, set_resolution=[630,512]
ENDELSE
my_hessi_ct,colors
TVLCT,r,g,b,/GET
...
IF KEYWORD_SET(counts) THEN BEGIN
	...
	CASE counts OF
		...
		3: BEGIN
		   ...
		   newrates=lonarr(4,ntimes)
		   newrates(0,*)=rates(0,*)+rates(1,*)
		   newrates(1,*)=rates(2,*)
		   newrates(2,*)=rates(3,*)+rates(4,*)
		   newrates(3,*)=rates(5,*)+rates(6,*)+rates(7,*)+rates(8,*)	
		   END
	...
	chrsiz=1.0
	ERASE
	utplot,uttimes,newrates(loc(0),*),xtitle='Start date: '+anytim(time_interval(0),/date_only,/ECS)+' UT', $
		ytitle='counts/s/detector',color=255, $
		title='HESSI Observing Summary Count Rates',charsize=chrsiz,xmargin=[10,2],ymargin=[4,2],_extra=_extra
	S=size(newrates)
	...
	CASE counts OF
		...
		3:  BEGIN
			...
			xyouts,0.825,0.88,charsize=1.0,'12-25 KeV',color=colors(2),/normal		    
			xyouts,0.825,0.85,charsize=1.0,'25-100 KeV',color=colors(3),/normal
		    xyouts,0.825,0.82,charsize=1.0,'0.1-17  MeV',color=colors(4),/normal
		    END
...
if KEYWORD_SET(mod_var) THEN BEGIN
	ERASE
	oso->plot,/mod_variance,_extra=_extra
	if KEYWORD_SET(filename) then BEGIN
			img=TVRD()
			if not keyword_set(jpg) then write_png,filename,img,r,g,b ELSE write_jpg,filename,img,r,g,b
				      ENDIF
...
if KEYWORD_SET(part_rates) THEN BEGIN
	...
	if bla(0) NE -1 then begin
		oso->plot,/particle_rate,_extra=_extra
		if KEYWORD_SET(filename) then BEGIN
				img=TVRD()
				if not keyword_set(jpg) then write_png,filename,img,r,g,b ELSE write_jpg,filename,img,r,g,b
		endif
