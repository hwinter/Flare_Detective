...
FUNCTION psh_trace_hessi_movie_make_cube, tracefiles, hessifile, obs_time_intv=obs_time_intv, max_time_diff=max_time_diff,min_nbr_counts=min_nbr_counts, ebands_ss=ebands_ss, dt=dt, NOFRAMENBR=NOFRAMENBR, drange=drange, EIT=EIT
	...
	;plotting images
		...
		IF KEYWORD_SET(EIT) THEN BEGIN
			IF EIT GE 2 THEN BEGIN
				eit_prep,tracefiles[tss],outheader,outimage,/no_calibrate	;,/no_index,/no_roll
				tracemap.DATA=outimage
			ENDIF
		ENDIF
		IF tracemap.ROLL_ANGLE eq -180. THEN BEGIN
			PRINT,'.......psh_trace_hessi_movie_make_cube.pro: ROLL_ANGLE is -180! Suspect problem. Putting it to 0...'
			tracemap.ROLL_ANGLE=0
			ROLL_ANGLE_PROBLEM=1
		ENDIF
		tracemap=map2earth(tracemap)

		IF KEYWORD_SET(drange) THEN BEGIN
			DMIN=MIN(tracemap.DATA)+drange[0]*(MAX(tracemap.DATA)-MIN(tracemap.DATA))
			DMAX=MIN(tracemap.DATA)+drange[1]*(MAX(tracemap.DATA)-MIN(tracemap.DATA))
		ENDIF ELSE BEGIN
			DMIN=MIN(tracemap.DATA)
			DMAX=MAX(tracemap.DATA)
		ENDELSE

		IF traceOK THEN plot_map,tracemap,bottom=8,/LOG,xrange=xrange,yrange=yrange,xtitle='',ytitle='',grid=10,/LIMB,xmar=[5,0],ymar=[2,2],/ISO,charsize=charsize,DMIN=DMIN,DMAX=DMAX $
