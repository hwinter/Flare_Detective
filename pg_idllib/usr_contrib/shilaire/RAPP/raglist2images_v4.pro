...
;=======================================================================================================================================
			B5data=rapp_get_spectrogram(time_interval,xaxis=B5x,yaxis=B5y,/NODECOMPRESS,/ANYTIM,archivedir='/ftp/pub/rag/callisto/observations/',valid_file_identifiers='*BLEN5M*l2.fit*', /BACK, /DESPIKE,/ELIM)
		...
		;Callisto BLEIN5m (if any)
			IF B5ok THEN rapp_spectro_plot,rapp_add_empty_channels(B5spg),xstyle=1,tit='CALLISTO -- BLEIEN 5m -- L2',ytit='Frequency [MHz]',xtit=' ',ymar=[0,2],xmar=[10,2],bottom=14,charsize=charsize,ystyle=1,timerange=time_interval,xtickname=REPLICATE(' ',10)
		;Callisto Sonnenturm (if any)
			IF ZSTSok THEN rapp_spectro_plot,rapp_add_empty_channels(ZSTSspg),xstyle=1,tit='CALLISTO -- ZSTS',ytit='Frequency [MHz]',xtit=' ',ymar=[0,2],xmar=[10,2],bottom=14,timerange=time_interval,xtickname=REPLICATE(' ',10),ystyle=1,charsize=charsize
		;Phoenix-2 spg	
			rapp_spectro_plot,image,anytim(time_interval[0],/date)+xaxis,yaxis,xstyle=1,tit='Phoenix-2 - STOKES I - Data min/max: '+strn(datamin,format='(f10.1)')+'/'+strn(datamax,format='(f10.1)')+' SFU',ytit='Frequency [MHz]',xtit=' ',ymar=[0,2],xmar=[10,2],bottom=14,timerange=time_interval,xtickname=REPLICATE(' ',10),charsize=1+(B5ok OR ZSTSok)
		;GOES lightcurves
