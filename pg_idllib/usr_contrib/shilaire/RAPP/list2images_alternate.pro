...
FOR i=0,nbursts-1 do begin
	...
	IF ((TODO NE 2) AND ( (anytim(time_interval(1))-anytim(time_interval(0))) LE MAXTIMEINTV) ) THEN BEGIN
		...
		image=rapp_get_spectrogram(time_interval,/ELIM,/BACK,xaxis=xaxis,yaxis=yaxis,avgback=avgback)
		backavg=avg(avgback)
		IF DATATYPE(image) EQ 'STR' THEN BEGIN
			PRINT,'......................No intensity (Stokes I or l1 or l2) rag fits files for this burst!'
			IF INDIVNOTIFY EQ 1 THEN BEGIN
				text='No intensity, i.e. Stokes I or l1 or l2, rag fits files for burst : '+anytim(time_interval(0),/ECS)
				cmd='echo '+text+' |mailx -s "Ragburst problem" shilaire'
				spawn,cmd	
			ENDIF
		ENDIF $
		ELSE begin
			PRINT,'.............................DOING STOKES I or LINEAR 1/2.'
...
;at this point, image is always an image in LINEAR SFU SCALE !!!
	...
	IF ((TODO NE 1) AND ( (anytim(time_interval(1))-anytim(time_interval(0))) LE MAXTIMEINTV) ) THEN BEGIN
		;do p files	(STOKES V)-----------------------------------------------------------------------------------------
		image=rapp_get_spectrogram(time_interval,/POL,/ELIM,/BACK,xaxis=xaxis,yaxis=yaxis)
		
		polct
		tvlct,r,g,b,/GET
	
		IF DATATYPE(image) EQ 'STR' THEN BEGIN
			PRINT,'......................No POLARISATION rag fits files for this burst!'
			IF INDIVNOTIFY EQ 1 THEN BEGIN
				text='No polarisation rag fits files for burst : '+anytim(time_interval(0),/ECS)
				cmd='echo '+text+' |mailx -s "Ragburst problem" shilaire'
				spawn,cmd	
			ENDIF
		ENDIF $
		ELSE begin
			...
			print,'Now, bad channels removal, polarization data has the following range: '
			print,'MIN: '+STRING(min(image))
			print,'MAX: '+STRING(max(image))
	...
	ENDIF ELSE print,'...... burst is longer than MAXTIMEINTV : not doing it...'
	NEXTBURSTPLEASE:
	IF BREAKONERROR NE 1 THEN CATCH,/CANCEL
ENDFOR
