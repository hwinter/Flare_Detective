...
;at this point, image is always an image in LINEAR SFU SCALE !!!
		;remove some bad pixels:  PSH 2002/06/25
			...
			charsize=2.
		;Phoenix-2 spg	
			rapp_spectro_plot,image,anytim(time_interval[0],/date)+xaxis,yaxis,xstyle=1,tit='Phoenix-2 - STOKES I - Data min/max: '+strn(datamin,format='(f10.1)')+'/'+strn(datamax,format='(f10.1)')+' SFU',ytit='Frequency [MHz]',xtit=' ',ymar=[1,2],xmar=[10,2],bottom=14,charsize=1
		;GOES lightcurves
			!P.MULTI=[2,1,4]
		...
		;RHESSI lightcurves
			...
			utplot,obstimes-obstimes[0],r1,obstimes[0],color=120,yr=[1,MAX([r1,r2,r3]) > 10],/YLOG,tit='RHESSI corrected countrates',xstyle=1,ytit='[cts/s/det]',charsize=charsize,xmar=[10,2],ymar=[2,0],xtit=''
			...
			flag_ss=[1,17,0]
			FOR i=0,N_ELEMENTS(flag_ss)-1 DO BEGIN
				CASE flag_ss[i] OF 
					1:BEGIN color=7 & ori=45 & END
					17:BEGIN color=4 & ori=0 & END
					ELSE: BEGIN color=2 & ori=90 & END
				ENDCASE				
				ss_intvs=psh_get_block_intv(flagdata[flag_ss[i],*],val=1)				
				IF ss_intvs[0] NE -1 THEN BEGIN
					label_plot,0.03,i+1,flaginfo.FLAG_IDS[flag_ss[i]],color=color,POLYFILL={ORIENTATION:ori}/LINE
					FOR j=0,N_ELEMENTS(ss_intvs[0,*])-1 DO POLYFILL,[flagtimes[ss_intvs[0,j]],flagtimes[ss_intvs[1,j]],flagtimes[ss_intvs[1,j]],flagtimes[ss_intvs[0,j]]]-flagtimes[0],10^[!Y.CRANGE[0],!Y.CRANGE[0],!Y.CRANGE[1],!Y.CRANGE[1]],/LINE_FILL,ORIENTATION=ori,color=color
				ENDIF				
			ENDFOR
			;v3: END------------------------------------------------------------------------------
