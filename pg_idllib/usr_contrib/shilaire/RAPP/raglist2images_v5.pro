...
;=======================================================================================================================================
		...
		;Callisto BLEIN5m (if any)
			IF B5ok THEN BEGIN
				...
				rapp_spectro_plot,thespg,xstyle=1,tit='CALLISTO -- BLEIEN 5m -- L2',ytit='Frequency [MHz]',xtit=' ',ymar=[0,2],xmar=[10,2],bottom=14,charsize=charsize,ystyle=1,timerange=time_interval,xtickname=REPLICATE(' ',10)
			ENDIF
		;Callisto Sonnenturm (if any)
			IF ZSTSok THEN BEGIN
				thespg=rapp_add_empty_channels(ZSTSspg)
				thespg.SPECTROGRAM = (-5) > thespg.SPECTROGRAM
				rapp_spectro_plot,thespg,xstyle=1,tit='CALLISTO -- ZSTS',ytit='Frequency [MHz]',xtit=' ',ymar=[0,2],xmar=[10,2],bottom=14,timerange=time_interval,xtickname=REPLICATE(' ',10),ystyle=1,charsize=charsize
			ENDIF
		;Phoenix-2 spg	
