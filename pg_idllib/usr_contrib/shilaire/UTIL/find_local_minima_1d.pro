...
FUNCTION find_local_minima_1d, vecteur, INTERP=INTERP,	$
	ss=ss
	...
	WHILE i LT n DO BEGIN
		CASE i OF
			0: BEGIN
				IF vector[0] LT vector[1] THEN BEGIN
			...
			END
			n-1: BEGIN
				IF vector[n-1] LT vector[n-2] THEN BEGIN
					IF datatype(res) EQ 'STR' THEN BEGIN
						res=vector[n-1] 
						ss=n-1
					ENDIF ELSE BEGIN
			...
			ELSE: BEGIN
				value=vector[i]
				IF ((vector[i-1] GT value) AND (vector[i+1] GT value))	THEN BEGIN
					IF datatype(res) EQ 'STR' THEN BEGIN
						res=vector[i] 
						ss=i
					ENDIF ELSE BEGIN
