...
PRO psh_plot_apar_png, PS=PS, GOES=GOES, V=V, file=file, CREATEFILE=CREATEFILE, L=L, kinkOKtime=kinkOKtime
...
;LC
		...
		IF KEYWORD_SET(PS) THEN utplot,os_times,rates1,yr=theminmax,/YLOG,ytit='Countrates !C[cts/s/det]',tit=anytim(obs_time_intv[0],/date,/ECS)+'  !7D!3t='+strn(times[N_ELEMENTS(times)-1]-times[0],format='(f10.1)')+' s',xtit='',xmar=xmar,ymar=[0,0],charsize=charsize,XTICKNAME=REPLICATE(' ',10) ELSE utplot,os_times,rates1,yr=theminmax,/YLOG,color=2,ytit='Countrates !C[cts/s/det]',xtit='',xmar=xmar,ymar=[0,1],charsize=charsize,xtickname=REPLICATE(' ',10)
		...
		IF KEYWORD_SET(PS) THEN outplot,os_times,rates2,linestyle=1 ELSE outplot,os_times,rates2,color=6
		IF KEYWORD_SET(PS) THEN plot_label,[0.025,-2],'12-25 keV',linestyle=1,/DEV,charsize=charsize/2. ELSE plot_label,[0.05,-2],'12-25 keV',color=6,/DEV,/NOLINE

		IF KEYWORD_SET(PS) THEN outplot,os_times,rates3,linestyle=2 ELSE outplot,os_times,rates3,color=255
		IF KEYWORD_SET(PS) THEN plot_label,[0.025,-3],'25-300 keV',linestyle=2,/DEV,charsize=charsize/2. ELSE plot_label,[0.05,-3],'25-300 keV',color=255,/DEV,/NOLINE
;-------------------------------------------------------------
...
;-------------------------------------------------------------
	FOR i=0,5 DO BEGIN
		...
		IF i EQ 5 THEN BEGIN
			utplot,times,fitpar[i,*],ytit=ytit,tit='',xtit='',xmar=xmar,ymar=[2,0],charsize=charsize,psym=psym
		ENDIF ELSE BEGIN
			IF ytit NE '' THEN utplot,times,fitpar[i,*],ytit=ytit,tit='',xtit='',xmar=xmar,ymar=[0,1],charsize=charsize,xtickNAME=REPLICATE(' ',10),psym=psym
		ENDELSE
		IF i EQ 0 AND KEYWORD_SET(GOES) THEN BEGIN
			outplot,goes_times,goes_em,color=2
			plot_label,/DEV,[0.025,-1],'RHESSI',charsize=charsize/2
			plot_label,/DEV,[0.025,-2],'GOES',charsize=charsize/2,color=2		
		ENDIF
		IF i EQ 1 AND KEYWORD_SET(GOES) THEN BEGIN
			outplot,goes_times,goes_t/11.594,color=2
			plot_label,/DEV,[0.025,2],'RHESSI',charsize=charsize/2
			plot_label,/DEV,[0.025,1],'GOES',charsize=charsize/2,color=2		
		ENDIF
...
;-------------------------------------------------------------
	;kinetic power
		...
		utplot,times,power*power_corr[0,*],ytit='Non-thermal !Cpower [erg/s]',charsize=charsize,xmar=xmar,ymar=[0,0],/YLOG,xtit='',XTICKNAME=REPLICATE(' ',10)	;,yr=[1e27,1e31]
	...
	;plotting the whole business...
		...
		utplot,times,Ekin_corr,ytit='Energies !C[erg]',charsize=charsize,xmar=xmar,ymar=[1,0],/YLOG,xtit='',XTICKNAME=REPLICATE(' ',10)	;,yr=minmax([Ekin,Eth,goes_Eth])
		...
		outplot,goes_times,goes_Eth,color=2,linestyle=2
		plot_label,/DEV,[0.55,1],'thermal (GOES)',charsize=charsize/2,color=2,linestyle=2
