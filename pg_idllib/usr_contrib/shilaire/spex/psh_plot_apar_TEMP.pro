...
!P.MULTI=[2,1,(6+KEYWORD_SET(LC)+4*KEYWORD_SET(ENERGY)+KEYWORD_SET(DENSITY)+2*KEYWORD_SET(NUMBER))/2]
	;kinetic power
		...
		powerTO=nonthermal_power(fitpar[5,*],fitpar[2,*],10.,/TURNOVER)
	...
	IF KEYWORD_SET(CORRECTED) THEN BEGIN
		...
		IF NOT EXIST(power_corr) THEN power_corr=nonthermal_power_correction(fitpar[5,*]+1,10.,[10,35],/STD,/TOGETHER)
