...
PRO psh_plot_apar_paper_20040719, PS=PS, GOES=GOES, LC=LC, ENERGY=ENERGY, V=V, DENSITY=DENSITY, NUMBER=NUMBER, file=file, CREATEFILE=CREATEFILE, CORRECTED=CORRECTED, L=L,newcorr, newEto
...
;-------------------------------------------------------------
	FOR i=0,5 DO BEGIN
		CASE i OF
			...
			5:ytit='!7c!3!D!N'
			4:ytit='E!Dto!N [keV]'	;ytit='Low-E cutoff !C[keV]		
			ELSE:ytit=''		
		ENDCASE
		IF i EQ 4 THEN BEGIN
			utplot,times,newEto,ytit=ytit,tit='',xtit='',xmar=[8,1],ymar=[1,0],charsize=charsize,xtickNAME=REPLICATE(' ',10),psym=psym
		ENDIF ELSE BEGIN
			IF ytit NE '' THEN utplot,times,fitpar[i,*],ytit=ytit,tit='',xtit='',xmar=[8,1],ymar=[1,0],charsize=charsize,xtickNAME=REPLICATE(' ',10),psym=psym
		ENDELSE
		...
		ENDIF
		IF i EQ 4 THEN PLOTS,anytim('2002/11/10 '+['03:07:18','03:09:56']),[10,10],linestyle=2
	ENDFOR
...
;adding correction from non-ideal EphTO/Eto relation because of presence of (T,EM)... (No corrections yet for cutoff case...
		power_corr[1,*]=power_corr[1,*]*newcorr
		...
		utplot,times,powerTO*power_corr[1,*],ytit='Non-thermal !Cpower [erg/s]',charsize=charsize,xmar=[8,1],ymar=[1,0],/YLOG,yr=[1d27,1d29],xtit='',XTICKNAME=REPLICATE(' ',10),psym=psym
	...
	;plotting the whole business...
		...
		IF KEYWORD_SET(CORRECTED) THEN BEGIN
			...
			utplot,times,EkinTO_corr,ytit='Energies !C[erg]',charsize=charsize,xmar=[8,1],ymar=[1,0],/YLOG,xtit='',yr=yrng,psym=-psym	;,XTICKNAME=REPLICATE(' ',10)	;,yr=[1d27,1d31])
			plot_label,/DEV,[0.025,-1],'E!Dnth!N',charsize=charsize/2,linestyle=0,psym=-psym
		ENDIF

		outplot,times,Eth,linestyle=0
		plot_label,/DEV,[0.025,-2],'E!Dth,RHESSI!N',linestyle=0,charsize=charsize/2
		outplot,goes_times,goes_Eth,linestyle=2
		plot_label,/DEV,[0.025,-3],'E!Dth,GOES!N',linestyle=2,charsize=charsize/2	
