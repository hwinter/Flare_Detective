...
;	B2_n: (B.sin(psi))^2/n, in [Gauss^2.cm^3] if used, will include Magnetobremsstrahlung (gyrosynchroton losses) in the collisional energy-loss cross-section. Assumes constant density n!
...
;		if nothing (default): standard power-law, with spectral index delta=4, A50=1.295d33 el/s/keV
...
;	2004/04/05: added B2_n (Magnetobremsstrahlung) keyword...
;
;--------------------------------------------------------------------------------------------
;--------------------------------------------------------------------------------------------
...
FUNCTION forward_spectrum__number_of_photons__compute, E0, n_pts
	...
	y=forward_spectrum_thick__brm_crosssection(E)/E/collisional_energy_loss_crosssection(E,FS_PARAMS.il,T=FS_PARAMS.targetT,BETHEBLOCH=(FS_PARAMS.COLLCROSS EQ 1), HOLMAN=(FS_PARAMS.COLLCROSS EQ 2), B2_n=FS_PARAMS.B2_n)
...
FUNCTION forward_spectrum_thick__integrand, E,E0
	COMMON FS_COMMON, FS_PARAMS, A
	RETURN, forward_spectrum_thick__electron_injection(E0)*forward_spectrum_thick__brm_crosssection(E)/E/collisional_energy_loss_crosssection(E,FS_PARAMS.il,T=FS_PARAMS.targetT,BETHEBLOCH=(FS_PARAMS.COLLCROSS EQ 1), HOLMAN=(FS_PARAMS.COLLCROSS EQ 2), B2_n=FS_PARAMS.B2_n)
END
;---------------------------------------------------------------------------------------------------
;---------------------------------------------------------------------------------------------------
...
;---------------------------------------------------------------------------------------------------
;---------------------------------------------------------------------------------------------------
FUNCTION forward_spectrum_thick,EARTH=EARTH,LOUD=LOUD, il=il, targetT=targetT, Estar=Estar, ALBEDO=ALBEDO, thermal=thermal, Eph=Eph, E0max=E0max, PTS=PTS, boundaries=boundaries, SCREENING=SCREENING, HAUG=HAUG, KRAMER=KRAMER, RIEMANN=RIEMANN,el_spectrum=el_spectrum, BETHEBLOCH=BETHEBLOCH, HOLMAN=HOLMAN, B2_n=B2_n

	COMMON FS_COMMON, FS_PARAMS, FS_INJECTION
	FS_PARAMS={FS_PARAMS, LOUD:0, il:-1d, targetT:-1d, E0max:-1d, curEph:-1d, SCREENING:0, BREMCROSS:0, COLLCROSS:0, B2_n:0d}
	FS_INJECTION={delta:4d,A50:1.295d33,Eto:-1d,Ebump:-1d,Eco:-1d,thermal:[-1d,-1d]}
	...
	IF EXIST(E0max) THEN FS_PARAMS.E0max=DOUBLE(E0max) ELSE BEGIN
		IF KEYWORD_SET(RIEMANN) THEN FS_PARAMS.E0max=1d4 ELSE FS_PARAMS.E0max=1d4
	ENDELSE
	...
	IF KEYWORD_SET(B2_n) THEN FS_PARAMS.B2_n=B2_n
	IF KEYWORD_SET(el_spectrum) THEN BEGIN
		IF tag_exist(el_spectrum,'delta',/QUIET) THEN FS_INJECTION.delta=el_spectrum.delta
		IF tag_exist(el_spectrum,'A50',/QUIET) THEN FS_INJECTION.A50=el_spectrum.A50
		IF tag_exist(el_spectrum,'A',/QUIET) THEN FS_INJECTION.A50=el_spectrum.A/50^FS_INJECTION.delta
		IF tag_exist(el_spectrum,'Ae',/QUIET) THEN FS_INJECTION.A50=el_spectrum.Ae/50^FS_INJECTION.delta
		IF tag_exist(el_spectrum,'Eto',/QUIET) THEN FS_INJECTION.Eto=el_spectrum.Eto
		IF tag_exist(el_spectrum,'Ebump',/QUIET) THEN FS_INJECTION.Ebump=el_spectrum.Ebump
		IF tag_exist(el_spectrum,'Eco',/QUIET) THEN FS_INJECTION.Eco=el_spectrum.Eco
		IF tag_exist(el_spectrum,'thermal',/QUIET) THEN FS_INJECTION.thermal=el_spectrum.thermal		
	ENDIF
	...
	CASE N_ELEMENTS(boundaries) OF
		0: BEGIN
			...
			IF tag_exist(el_spectrum,'Eto',/QUIET) THEN boundaries=[boundaries,el_spectrum.Eto]
			IF tag_exist(el_spectrum,'Ebump',/QUIET) THEN boundaries=[boundaries,el_spectrum.Ebump]
			IF tag_exist(el_spectrum,'Eco',/QUIET) THEN boundaries=[boundaries,el_spectrum.Eco]								
