...
;
; $Id: pg_plotgoes.pro 45 2008-01-11 15:57:15Z pgrigis $
;-
...
loadct,0
pg_plotgoes,time,/ylog,channel=1,background=255,color=0 $
            ,yrange=[1d-7,20d-4],/ystyle,/xstyle,title='Paolo Grigis GOES plotter'
loadct,5
pg_plotgoes,time,/ylog,channel=2,background=255,color=28,/overplot,thick=2
pg_plotgoes,time,/ylog,channel=1,background=255,color=128,/overplot,thick=2
...
;-----------------------------------------------------
   ...
   data=a->getdata(sdac=sdac,/three,/struct)
   ...
   IF NOT keyword_set(noplot) THEN BEGIN
      ...
      IF channel NE 3 THEN BEGIN
         ...
         ELSE BEGIN
            ...
            utplot,anytim(time_intv)-anytim(time_intv[0]),[0,0],anytim(time_intv[0]),ytitle=ytitle,title=title, $
                   yrange=[ymin,ymax],ytickformat='tickgoes',_extra=e,/nodata

            outplot,timegoes-timegoes[0],datagoes,timegoes[0]
