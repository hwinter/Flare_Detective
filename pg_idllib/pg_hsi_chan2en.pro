...
;       detectors 1F 3F 4F 5F 6F 7F 8F 9F, or
;       1R 3R 4R 5R 6R 7R 8R 9R if /rear is set
;
; CALLING SEQUENCE:
;
...
;       
; KEYWORDS:
;       rear: if set, compute the REAR segments energy instead of
;       front (which is the default)
;
; EXAMPLE:
;
;        energy=pg_hsi_chan2en([50,60,70],time_intv='26-FEB-2002 '+['10:25','10:30'])
;       
...
;                   if chan is a scalar!
;       30-APR-2004 added keyword rear, renamed PG
;
; AUTHOR
...
FUNCTION pg_hsi_chan2en,chan,time_intv=time_intv,rear=rear

IF max(chan) GE 8192 OR min(chan) LT 0 THEN BEGIN
   print,'Channel have to be in the 0-8191 range!'
   return,-1 
ENDIF
...
;get energy edges
IF NOT keyword_set(rear) THEN $
   energy_edges = hsi_get_e_edges(a2d_index=[0,2,3,4,5,6,7,8] $
                               ,time_wanted=time_intv,/new_gain,/twod) ELSE $
   energy_edges = hsi_get_e_edges(a2d_index=9+[0,2,3,4,5,6,7,8] $
