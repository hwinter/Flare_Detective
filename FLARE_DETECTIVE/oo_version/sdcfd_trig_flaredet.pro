...
; PROCEDURE:
;
;     This programs need to work incrementally: i.e. it is called every time a
...
;     complex:
;
;     -if a new pixels flares, it is a "primary" flares if there are no close
;         flares active, and a "secondary" flare otherwise.
;
;     -a new primary flare causes a new event to "open"
;
;     -when a secondary flare ends, no special action is taken
;
;     -when a primary flare ends, either active secondaries are promoted to
...
; detect flares 
   ...
   ENDIF $ 
   ELSE BEGIN 
      help, imageFilename, events,numActiveEvents, imageRejected
